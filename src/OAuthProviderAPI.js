...
export async function authorize(request, env, KV, OIDC_KEY_PAIR) {
	if (request.method != 'GET') return new Response("400 Bad request. The appropriate HTTP method is 'GET'.", { status: 400 });
	...
	if (!client_id) {
		return new Response(
			`{"error":"invalid_client","error_description":"client_id must be present. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	if (!response_type) {
		return new Response(
			`{"error":"invalid_request","error_description":"response_type must be present. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	...
	if (!OAuthClient)
		return new Response(`{"error":"invalid_request","error_description":"404 Client does not exist. Check your client_id field."}`, {
			status: 404,
		});
	if (OAuthClient.client_type == 'public' && response_type == 'code')
		return new Response(
			`{"error":"invalid_request","error_description":"400 A public client can not request a code. It must user the implict/token protocol."}`,
			{ status: 400 },
		);
	if (OAuthClient.client_type == 'confidential' && response_type == 'token')
		return new Response(
			`{"error":"invalid_request","error_description":"400 A confidential client can not request a token. It must use code protocol."}`,
			{ status: 400 },
		);
	...
	if (redirectionURIs || redirect_uri) {
		if (!redirectionURIs && redirect_uri) {
			...
			if (redirect_uri && redirect_uri != redirectionURIs[0])
				return new Response(
					`{"error":"invalid_request","error_description":"redirect_uri is incorrect. Must have one valid redirect_uri. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
					{ status: 400 },
				);
			verifiyedRedirectURI = true;
			...
			return new Response(`{"error":"invalid_request","error_description":"redirect_uri does not equal any registered URI."}`, {
				status: 400,
			});
		}
	} else {
	...
	if (!verifiyedRedirectURI)
		return new Response(
			`{"error":"server_error","error_description":"Sorry, the server ran into an unexpected edge case. Please contact an admin"}`,
			{ status: 500 },
		);
	if (!query.get('scope'))
	...
	if (scopes.length == 0) return new Response(`{"error":"invalid_scope","error_description":"Scope can not be nothing."}`, { status: 400 });
	...
	if (response_type == 'code') {
		...
		const claims = {};
		if (!scopes.includes('openid')) return new Response("400 Bad Request. Must have scope 'openid' to use id_token.", { status: 400 });
...
export async function token(request, env, KV, OIDC_KEY_PAIR) {
	if (request.method != 'POST') return new Response("405 Method Not Allowed. The appropriate HTTP method is 'POST'.", { status: 405 });
	...
	if (!code) {
		return new Response(
			`{"error":"invalid_request","error_description":"code must be present. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	if (!stateJson) {
		return new Response(`{"error":"invalid_request","error_description":"code has been used, expired, or never existed."}`, {
			status: 400,
		});
	}
	if (redirect_uri != stateJson.redirect_uri) {
		return new Response(
			`{"error":"invalid_request","error_description":"redirect_uri must equal the one used to get the 'code'. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	if (grant_type != 'authorization_code') {
		return new Response(
			`{"error":"invalid_request","error_description":"grant_type must be 'authorization_code'. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	...
	if (!client_secret) {
		return new Response(
			`{"error":"invalid_client","error_description":"client_secret must be present. OAuth 2.0 spec: https://datatracker.ietf.org/doc/html/rfc6749"}`,
			{ status: 400 },
		);
	}
	if (client_id != stateJson.client.client_id) {
		return new Response(
			`{"error":"invalid_client","error_description":"client_id used to gain 'code' must be the same as the one exchangeing for an access_token."}`,
			{ status: 400 },
		);
	}
...
export async function tempToken(request, env, KV) {
	...
	if (request.method != 'GET') return new Response("405 Method Not Allowed. Try using 'GET'", { status: 405 });
	...
	if (!client) return new Response('404 Not Found. That client does not exist.', { status: 404 });
	if (client.ownerUserID != user.userID) return new Response('401 Unauthorized. User/Sessions does not own that client.', { status: 401 });
