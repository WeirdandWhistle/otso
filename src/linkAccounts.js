...
export async function linkAccounts(request, env, KV){
	if(request.method == 'GET'){
		...
		if(!OAuthState.link)
			return new Response('400 Bad Request. forgot GET request before confirming.',{status:400});
		...
		if(!validUsername(username))
			return new Response('400 Bad Request. username is not valid.',{status: 400});
