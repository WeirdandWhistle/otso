...
export async function client(request, env, KV) {
	...
	if (request.method == 'POST') {
		...
		if (!validUsername(json.name)) return new Response('400 Bad Request. Name is not valid.', { status: 400 });
		try {
			const url = new URL(json.redirect_uri);
			if (url.protocol != 'https:' && url.hostname != 'localhost')
				return new Response("400 Bad Request. Must use 'https' OR 'localhost'. If you used '127.0.0.1' just change it to 'localhost'.", {
					status: 400,
				});
		} catch (error) {
		...
		if (oldClient) return new Response('400 Bad Request. Client name is already in use.', { status: 400 });
		...
		if (!json.client_id) return new Response('400 Bad Request. Missing the client_id parameter.', { status: 400 });

		const client = await db.getOAuthClientFromClientID(env, json.client_id);
		if (!client) return new Response('400 Bad Request. Client does not exist.');
		if (client.ownerUserID != user.userID) return new Response('400 Bad Request. User does not own client.', { status: 401 });
		...
		if (!json.client_id) return new Response('400 Bad Request. Missing client_id param.', { status: 400 });

		const client = await db.getOAuthClientFromClientID(env, json.client_id);
		if (!client) return new Response('400 Bad Request. Client does not exists', { status: 400 });
