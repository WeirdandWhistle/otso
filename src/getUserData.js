...
export async function getGithubUser(access_token) {
	...
	if (!res.ok) throw new Error(`Github /user API failed with code ${res.status}. Text:`, await res.text());
...
export async function getSlackUser(tokens) {
	...
	if (!slackInfo.ok) throw new Error(`Slack API failed with status ${slackInfo.status}. Text: ` + (await slackInfo.text()));
...
export async function getDiscordUser(access_token) {
	...
	if (!discordInfo.ok) throw new Error(`Discord API failed with status code ${discordInfo.status}. Text: ` + (await discordInfo.text()));
...
export async function getTwitchUser(access_token, client_id) {
	...
	if (!twitchInfo.ok) throw new Error(`Twitch API failed with status code ${twitchInfo.status}. Text: ` + (await twitchInfo.text()));
