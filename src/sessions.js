...
export function getSessionID(request) {
	...
	if (!cookiesArray) return null;
	let sessionID;
	for (const cookie of cookiesArray) {
		if (cookie.split('=')[0] == 'session') sessionID = cookie.split('=')[1];
	}
...
export async function useCSRFToken(request, env, KV) {
	...
	if (!safeCompareString(data, `${token}.${user.userID}.${sessionID}`)) return false;
	return true;
}
export async function revokeSessionAPI(request, env, KV) {
	if ((await useCSRFToken(request, env, KV)) != true) return new Response('401 Unauthorized. Wrong CSRFToken.', { status: 401 });
	const user = await getUserIfSession(request, env);
...
export async function CSRFTokenEndpoint(request, env, KV) {
	...
	const user = await getUserIfSession(request, env);
	if (!user) return new Response('401 Unauthorized. Invalid session.', { status: 401 });
