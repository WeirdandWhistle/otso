...
export async function authorizeApp(request, env, KV) {
	...
	if (request.method != 'POST') return new Response("405 Method Not Allowed. Try using 'POST'", { status: 405 });
	...
	if (!requestJson.client_id) return new Response("400 Bad Request. This endpoint requires a 'client_id' in the JSON.", { status: 400 });

	const client = await db.getOAuthClientFromClientID(env, requestJson.client_id);
	if (!client) return new Response('400 Bad Request. Client does not exist.', { status: 400 });
